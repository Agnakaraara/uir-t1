...
class HexapodHAL:
    ...
    def get_laser_scan(self):
        ...
        if self.laser_init:
            errorCode, signalVal = vrep.simxGetStringSignal(self.clientID, "MySignal"+ str(self.robot_id), vrep.simx_opmode_streaming) #start streaming
            self.laser_init = False
        else:
            errorCode, signalVal = vrep.simxGetStringSignal(self.clientID, "MySignal"+ str(self.robot_id), vrep.simx_opmode_buffer) #fetch new data from stream
        ...
        if data and data[0]==self.robot_id and data[3] > 0:
            scan_msg = LaserScan()
            scan_msg.header.timestamp = data[1]
            scan_msg.angle_max = (data[2]/2.0)/180.0*math.pi
            scan_msg.angle_min = -(data[2]/2.0)/180.0*math.pi
            scan_msg.range_min = 0.01
            scan_msg.range_max = 10.0
            scan_msg.angle_increment = 1.0/data[3]/180*math.pi
            scan_msg.distances = data[4:]
            return scan_msg
        else:
            return None
